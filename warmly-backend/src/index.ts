...
// In-memory data for MVP (Prisma to be added later)
...
const WARM_MESSAGES: WarmMessage[] = [];

function seedPhrases() {
  if (PHRASES.length) return;
  const ru: Record<DayPart, string[]> = {
    morning: [
  ...
  (['ru', 'en'] as LanguageCode[]).forEach((lang) => {
    (['morning', 'day', 'evening'] as DayPart[]).forEach((part) => {
...
app.get('/api/phrases', (req, res) => {
  ...
  if (part) list = list.filter((p) => p.part === part);
  // random sample
...
app.post('/api/favorites', (req, res) => {
  ...
  const fav: Favorite = { id: uuidv4(), deviceId, phraseId, createdAt: new Date().toISOString() };
  FAVORITES.push(fav);
...
app.post('/api/warm', (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json(parsed.error.flatten());
  const msg: WarmMessage = { id: uuidv4(), createdAt: new Date().toISOString(), ...parsed.data };
